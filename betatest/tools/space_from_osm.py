"""space_from_osm.py -- turn vendored OpenStreetMap geometry into a playable Space.
    ...
    py -3.11 betatest/tools/space_from_osm.py macroplaza-mall --era=1985   (with content/geo/macroplaza-mall.1985.json)
...
def load_era(slug, era):
    """content/geo/<slug>.<era>.json -- what was different then. Additive, sourced."""
    if not era:
        return None
    path = CONTENT / "geo" / f"{slug}.{era}.json"
    if not path.exists():
        raise SystemExit(f"no era overlay at {path}")
    return json.load(open(path, encoding="utf-8"))


def build(slug, era=None):
    src = json.load(open(CONTENT / "geo" / f"{slug}.osm.json", encoding="utf-8"))
    overlay = load_era(slug, era)
    els = src["elements"]
    if overlay:
        # Things that did not exist yet are dropped; things that had another
        # name get it back. Every entry in the overlay carries its source.
        gone = {(r["type"], r["id"]) for r in overlay.get("remove", [])}
        els = [e for e in els if (e["type"], e["id"]) not in gone]
        for r in overlay.get("rename", []):
            for e in els:
                if e["type"] == r["type"] and e["id"] == r["id"]:
                    e["tags"]["name"] = r["name"]
    park = src["park"]
    ...
    if overlay:
        for x in overlay.get("examine", []):
            if "lat" in x:
                x["x"], x["y"] = T((x["lat"], x["lon"]))
            item = {"id": x["id"], "type": "examine", "x": x["x"], "y": x["y"], "label": x["label"], "text": x["text"]}
            if x.get("source"):
                item["source"] = x["source"]
            interactables.append(item)

    era_id = overlay["era"] if overlay else None
    space = {
        "schema": 1,
        "id": f"space:{slug}" + (f"-{era_id}" if era_id else ""),
        "name": (overlay.get("name") if overlay else None) or park["name"],
        "kind": "exterior",
        "theme": "street",
        "canon": (overlay.get("canon") if overlay else None) or "documentary",
        "era": era_id or "present",
        "era_label": (overlay.get("label") if overlay else None) or "Now",
        "era_style": (overlay.get("style") if overlay else None),
        "era_date": (overlay.get("date") if overlay else None),
        "era_date_precision": (overlay.get("date_precision") if overlay else None),
        "era_valid": (overlay.get("valid") if overlay else None),
        "era_note": (overlay.get("note") if overlay else None),
        "era_sources": (overlay.get("sources") if overlay else None),
    ...
    out = CONTENT / "spaces" / (f"{slug}-{era_id}.json" if era_id else f"{slug}.json")
...
if __name__ == "__main__":
    ...
    era = next((a.split("=", 1)[1] for a in sys.argv[1:] if a.startswith("--era=")), None)
    build(args[0] if args else "discovery-green", era)
